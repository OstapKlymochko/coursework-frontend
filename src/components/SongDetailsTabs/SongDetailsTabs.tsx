...
export const SongDetailsTabs = () => {
    ...
    useEffect(() => {
        console.log((commentsListStruct.comments.length / 10) < commentsListStruct.pagesCount)
    }, [commentsListStruct.comments.length])
        ...
        <Box sx={{width: '100%'}}>
            ...
            <CustomTabPanel value={value} index={0}>
                <Box height={'70vh'} sx={{position: 'relative', overflowY: 'scroll'}}>
                    {trackList.songs.map(s => <SmallSongCard key={generateUniqueID()} song={s}/>)}
            ...
            <CustomTabPanel value={value} index={1}>
                <Box height={'70vh'} sx={{position: 'relative', overflowY: 'scroll'}}>
                    ...
                    {!!commentsListStruct.comments.length && commentsListStruct.comments.map(c =>
                        <Comment key={generateUniqueID()} comment={c} handleReply={handleReply}
