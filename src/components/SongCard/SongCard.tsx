...
export const SongCard: FC<IProps> = ({song}) => {
    ...
    const {currSong, isPlaying} = useAppSelector(s => s.playerReducer);
    const dispatch = useAppDispatch();

    const getSongDetails = () => {
        if (!currSong || currSong.id !== song.id) dispatch(songsActions.getSongById(song.id)).unwrap().then(data => {
            dispatch(playerActions.setSong({...song, ...data}));
            dispatch(playerActions.play())
        }).catch(() => {
        });
        else if (isPlaying && currSong.id === song.id) {
    ...
    const StateIcon = isPlaying && !!currSong && currSong.id === song.id ? PauseIcon : PlayArrow
    return (
